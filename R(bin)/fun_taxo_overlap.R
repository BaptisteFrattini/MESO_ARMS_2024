#' Abundance  & occurence classsification of MSP
#'
...
#' @export
#'
fun_taxo_overlap <- function(data_and_meta_clean){
  ...
  # Step 2: Identify the rows belonging to each campaign
  p50arms_rows <- meta_mean$arms[meta_mean$campain == "P50ARMS"]
  runarms_rows <- meta_mean$arms[meta_mean$campain == "RUNARMS"]
  
  # Step 3: Filter the rows in `data_mean` corresponding to each campaign
  p50arms_data <- data_mean[rownames(data_mean) %in% p50arms_rows, ]
  runarms_data <- data_mean[rownames(data_mean) %in% runarms_rows, ]
  ...
  # p50arms_abundance <- colMeans(p50arms_data)
  # runarms_abundance <- colMeans(runarms_data)
  # 
  # Step 5: Combine into a single data frame
  species_abundance <- data.frame(
    Species = colnames(data_mean),
    P50ARMS = p50arms_abundance,
    RUNARMS = runarms_abundance,
    row.names = NULL
  )
  ...
  species_abundance$Status <- ifelse(
    ...
    "absent", 
    ifelse(
      species_abundance$P50ARMS > 1.0 & species_abundance$RUNARMS == 0, 
      "Common deep exclusive", 
      ifelse(
        species_abundance$RUNARMS > 1.0 & species_abundance$P50ARMS == 0, 
        "Common shallow exclusive", 
        ifelse(
          species_abundance$P50ARMS > 1.0 & species_abundance$RUNARMS > 1.0, 
          "Common generalist", 
          ifelse(
            species_abundance$P50ARMS < 1.0 & species_abundance$RUNARMS == 0, 
            "Rare deep exclusive", 
  ...
  g1 <- ggplot(species_long, aes(x = reorder(Species, -Percentage), y = Percentage, fill = Status)) +
    ...
    labs(
      ...
      x = "Species",
      y = "log(1 + Species relative abundance)"
    ) +
    theme_minimal()
  
  g1
  ggsave("outputs/taxo_overlap_barplot.pdf", g1, width = 9, height = 15 )
  
  species_abundance$ab_total <- species_abundance$P50ARMS + species_abundance$RUNARMS
  
  species_abundance_sorted <- species_abundance %>%
    arrange(ab_total)
  ...
  ggsave("outputs/Cumul_hist.pdf", cumulative_histogram, width = 8, height = 5 )
