...
#' @export
#'
fun_map_arms_scale <- function(data_and_meta_clean_fullsites, gps_sites, runa_map, roda_map, runa_reef, roda_reef){
  ...
  colnames(meta) <- c("name", "campain", "triplicat", "site", "island")
  ...
  richesse <- vegan::specnumber(mat_species, groups = data_with_meta$triplicat)
  ...
  spe.beta <- adespatial::LCBD.comp(matrix.bray)
  
  spe.beta.bis <- adespatial::beta.div(matrix.bray)
  spe.beta$LCBD
  ...
  make_maps_for_group <- function(group_name, rare_species_df) {
    ...
    matrix.bray <- vegan::vegdist(data_mean_group, method = "bray")
  ...
  ggsave("outputs/Cartes - LCBD_Rarity/final_map_arms_scale.pdf", plot = final_plot, width = 0.75*18, height = 18)
