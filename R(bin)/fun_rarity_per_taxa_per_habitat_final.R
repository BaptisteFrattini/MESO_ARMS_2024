...
fun_rarity_per_taxa_per_habitat_fullsites <- function(data_and_meta_clean_fullsites){
  ...
  #### Step 4: Compute Rarity threshold per taxa ####  
  
  #Ascidians - p50a
  ascidiacea_species <- corr_taxa$Species[corr_taxa$taxa == "Ascidiacea"]
  data_ascidiacea_p50a <- rodarms_presence[, colnames(rodarms_presence) %in% ascidiacea_species]
  ...
  data_porifera_p50a <- rodarms_presence[, colnames(rodarms_presence) %in% porifera_species]
  ...
  data_bryozoa_p50a <- rodarms_presence[, colnames(rodarms_presence) %in% bryozoa_species]
  ...
  #### Step 5: Fonctions  pour calculer la rareté pour chaque groupe ####
  compute_rarity_roda <- function(species_list, group_name, threshold) {
    df <- rodarms_presence[, colnames(rodarms_presence) %in% species_list]
    freq <- colSums(df) / nrow(df)
    # freq <- freq[freq != 0]
  ...
  # Par groupe avec seuils spécifiques
  out_asc_runa <- compute_rarity_runa(ascidiacea_species, "Ascidiacea", threshold_asc_runa)
  out_asc_roda <- compute_rarity_roda(ascidiacea_species, "Ascidiacea", threshold_asc_roda)
  
  
  out_por_runa <- compute_rarity_runa(porifera_species, "Porifera", threshold_por_runa)
  out_por_roda <- compute_rarity_roda(porifera_species, "Porifera", threshold_por_roda)
  
  out_bry_runa <- compute_rarity_runa(bryozoa_species, "Bryozoa", threshold_bry_runa)
  out_bry_roda <- compute_rarity_roda(bryozoa_species, "Bryozoa", threshold_bry_roda)
  ...
  out_others_runa <- compute_rarity_runa(autres_groupes, "Other", threshold_all_species_runa)
  out_others_roda <- compute_rarity_roda(autres_groupes, "Other", threshold_all_species_roda)
