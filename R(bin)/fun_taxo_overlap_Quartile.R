#' Aoccurence classification of MSP basedon quartile
#'
...
#' @export
#'
fun_taxo_overlap_quart <- function(data_and_meta_clean){
  ...
  thresh_run <- species_abundance_run_sorted$ab_total[0.33*length(species_abundance_run_sorted$Species)]+0.0001

  species_abundance_p50_sorted <- species_abundance %>%
    arrange(P50ARMS)
  species_abundance_p50_sorted <- subset(species_abundance_p50_sorted, P50ARMS != 0)

  thresh_p50 <- species_abundance_p50_sorted$ab_total[0.33*length(species_abundance_p50_sorted$Species)] +0.0001
  ...
  species_abundance$Status <- ifelse(
    ...
    "absent", 
    ifelse(
      species_abundance$P50ARMS > thresh_p50 & species_abundance$RUNARMS == 0, 
      "Common deep exclusive", 
      ifelse(
        species_abundance$RUNARMS >  thresh_run & species_abundance$P50ARMS == 0, 
        "Common shallow exclusive", 
        ifelse(
          species_abundance$P50ARMS > thresh_p50 & species_abundance$RUNARMS >  thresh_run, 
          "Common generalist", 
          ifelse(
            species_abundance$P50ARMS < thresh_p50 & species_abundance$RUNARMS == 0, 
            "Rare deep exclusive", 
  ...
  g1
  ggsave("outputs/taxo_overlap_barplot_quart.pdf", g1, width = 9, height = 15 )
  ...
  ggsave("outputs/Cumul_hist_quart.pdf", cumulative_histogram, width = 8, height = 5 )
